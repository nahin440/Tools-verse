...
const OVERRIDES = {
  "change-video-fps": {
    ...
    longDescription: "<h2>Why change video frame rate with Tools Versa</h2><p>Frame rate mismatches cause real problems — combining footage from two different cameras with different fps, meeting a specific platform's technical requirements, or reducing file size by lowering an unnecessarily high frame rate for content that doesn't need it.</p><h2>Real re-encoding, duration and sync preserved</h2><p>Changing frame rate through actual re-encoding, rather than simply relabeling metadata, means the output genuinely plays at the new frame rate while keeping the video's length and audio synchronization correct.</p><h2>Common use cases</h2><p>Matching frame rates between clips from different cameras before editing them together, reducing an unnecessarily high frame rate to save file size, converting footage to a cinematic 24fps look, or meeting a specific platform's frame rate requirement for upload.</p>",
  },
  "compress-image": {
    howItWorks: {
  ...
  "pdf-to-word": {
    ...
    longDescription: "<h2>Why convert PDF to Word with Tools Versa</h2><p>A PDF is easy to view but hard to edit — pulling text out to revise a contract, update a report, or repurpose content in a new document usually means retyping everything by hand unless the conversion tool actually understands the PDF's structure.</p><h2>Deep content extraction, not a flat text dump</h2><p>This uses a PDF parsing engine capable of deep content-model extraction: per-run font styling, real embedded images at their correct position, and genuine table detection — producing an editable document that looks like the source, not a wall of unformatted text. If the source is a scan rather than real text, on-device OCR runs automatically first.</p><h2>Common use cases</h2><p>Editing an old contract that only exists as a PDF, updating a report that was exported to PDF without keeping the original source file, extracting content from a scanned document into an editable format, or repurposing a PDF's content into a new document.</p>",
  },
  "powerpoint-to-pdf": {
    howItWorks: {
  ...
  "remove-background": {
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          "Yes — this is the one tool on Tools Versa where the recognition model itself (not your photo) downloads from a third-party CDN the first time you use it, since the model is too large to bundle with the page for every visitor. After that first download, it's cached for reuse.",
      },
      {
